...
class Zillow_Api {
	...
	private function convert_pro_review_xml_to_html( $xml )
	{
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <div class="elvtn-zillow-pro-reviews">
            ...
            <br/><a target="_blank" rel="noopener noreferrer"> <xsl:attribute name="href"><xsl:value-of select="//*[local-name()='profileURL']"/>/Reviews/?scrnnm=<xsl:value-of select="//*[local-name()='screenname']"/></xsl:attribute>Read all <xsl:value-of select="//*[local-name()='reviewCount']"/> reviews on Zillow</a></p>
            <xsl:for-each select="//*[local-name()='review']">
                <div class="elvtn-zillow-pro-review">
                    ...
                    <a><xsl:attribute name="href"><xsl:value-of select="reviewURL"/></xsl:attribute>Read more of this review on Zillow...</a>
...
STYLESHEET;
	...
	public function GetProReviews($params)
	{
		...
		$params['zws-id'] = $this->zws_id;
		$params['returnTeamMemberReviews'] = $params['members'];
		unset($params['members']);

		$url = 'https://www.zillow.com/webservice/ProReviews.htm';
		$urlArgs = add_query_arg( $params, $url);
		...
		{
			return "<p>Error retrieving Zillow reviews - E0042</p>";
		}
		else
		{
			return $this->convert_pro_review_xml_to_html($response['body']);
		}
